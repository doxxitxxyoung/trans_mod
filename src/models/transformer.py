...
"""
def conv1d(inputs, filters, kernel_size, name, reuse):
    with tf.variable_scope(name, reuse=reuse):
        W = tf.get_variable("w", shape = [kernel_size, inputs.get_shape()[-1], filters])
        b = tf.get_variable("b", shape = [filters])

    return tf.nn.conv1d(inputs, W) + b
"""
...
class Transformer(TextEncoder):
    def __init__(self, text_batch, e1_dist_batch, e2_dist_batch, ep_dist, seq_len_batch, lstm_dim, embed_dim, position_dim,
        ...
        self.w_kappa = tf.random_uniform(shape = [self.num_heads])
        self.w_kappa = tf.Variable(self.w_kappa/tf.reduce_sum(self.w_kappa))
        self.w_alpha = tf.random_uniform(shape = [self.num_heads])
        self.w_alpha = tf.Variable(self.w_alpha/tf.reduce_sum(self.w_alpha))
        ...
        for i in range(self.block_repeats):
            with tf.variable_scope("num_blocks_{}".format(i)):
                ...
                input_feats = self.multihead_attention(queries=input_feats,
                                                  ...
                                                  causality=False,
                                                  reuse=reuse)
                # input_feats is now h * (N,T,C/h) list of tensors.

                ### Multiply Kappa
                input_feats = [input_feats[h] * self.w_kappa[h] for h in range(self.num_heads)]
    ...
    def multihead_attention(self, queries,
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for i in range(num_heads):
                outputs[i] += tf.nn.dropout(Q_[i*batch_size:(i+1)*batch_size], dropout_rate)

            # Normalize
                outputs[i] = self.normalize(outputs[i]) # h * (N, T_q, C/h)
    ...
    def feedforward(self, inputs, num_heads,
                    num_units=[2048, 512],
                    scope = "multihead_attention",
                    reuse=None):

        '''Point-wise feed forward net. ---> Modified for Weighted Transformer

        Args:
          inputs: a List of h tensors with (N,T_q,C/h).
          num_heads : number of heads defines (h = 4)
          num_units: A list of two integers.
          scope: Optional scope for `variable_scope`.
          reuse: Boolean, whether to reuse the weights of a previous layer
            by the same name.
        ...
        '''
        with tf.variable_scope(scope, reuse=reuse):
            outputs = inputs # h * (N,T_q,C/h)
            layer_params = self.layer_str.split(',')
            for h in range(num_heads):
                with tf.variable_scope("head%d" % h, reuse = reuse):
                    for i, l_params in enumerate(layer_params):
                        ...
                        params = {"inputs": outputs[h], "filters": dim, "kernel_size": width,
                        "activation": tf.nn.relu,
                        "use_bias": True, "padding": "same", "dilation_rate": dilation}

                        outputs[h] = tf.layers.conv1d(**params)

                        """
                        outputs[h] = conv1d(outputs[h], filters = dim, kernel_size = width,
                                            name = "head%d_layer%d"%(h,i), reuse=reuse)
                        outputs[h] = tf.nn.relu(outputs[h])
                        """
                    # mask padding
                    outputs[h] *= tf.expand_dims(tf.cast(tf.not_equal(self.text_batch, self.pad_idx), tf.float32), [2])
                    # Residual connection  "name" : "head%d_layer%d" % (h,i),
                    inputs[h] += outputs[h]

                    # Normalize
                    outputs[h] = self.normalize(outputs[h])
